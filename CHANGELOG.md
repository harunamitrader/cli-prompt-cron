...
- **実行中プロセスの強制停止**: 実行中のジョブカードに「強制停止」ボタンを表示。暴走したプロセスをダッシュボードから kill 可能
- **実行中ステータス表示**: プロセス実行中のジョブに「実行中」バッジを表示
- **`PATCH /api/jobs/:name`**: ジョブの `active` フィールドをトグルする API エンドポイントを追加
- **`DELETE /api/running/:name`**: 実行中プロセスを強制停止する API エンドポイントを追加
