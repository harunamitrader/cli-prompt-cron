...
ジョブファイルは `~/.cli-prompt-cron/jobs/` に JSON 形式で保存されます。
...
`~/.cli-prompt-cron/jobs/<名前>.json` を作成します。
...
`~/.cli-prompt-cron/jobs/<名前>.json` を削除します。

```bash
rm ~/.cli-prompt-cron/jobs/<名前>.json
```

### 5. ジョブ一覧確認

```bash
ls ~/.cli-prompt-cron/jobs/
```
...
# ログ一覧
ls ~/.cli-prompt-cron/logs/
```

### 7. 実行結果確認

実行結果は `~/.cli-prompt-cron/results/` に JSON 形式で保存されます。
...
# 特定の結果を確認
cat ~/.cli-prompt-cron/results/<ファイル名>.json
```

### 8. ダッシュボード起動

```bash
node /path/to/cli-prompt-cron/start.js
```
