...
ジョブファイルは `./data/jobs/` に JSON 形式で保存されます。
変更は即座に反映されます（デーモン再起動不要）。

> **重要**: このスキルは cli-prompt-cron リポジトリのクローン先ディレクトリ内で実行してください。
> パスはすべてプロジェクトルートからの相対パス（`./data/`）です。
...
`./data/jobs/<名前>.json` を削除します。

```bash
rm ./data/jobs/<名前>.json
```
...
実行結果は `./data/results/` に JSON 形式で保存されます。
