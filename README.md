...
`~/.cli-prompt-cron/jobs/` に JSON ファイルを置くだけで AI CLI コマンドを定期実行できます。
...
- **ファイルベース管理** — `~/.cli-prompt-cron/jobs/` の JSON ファイルで完結。デーモン再起動不要のホットリロード対応
- **マルチ CLI 対応** — Claude Code / Gemini CLI / Codex すべてに対応
- **結果の永続保存** — 実行結果を `~/.cli-prompt-cron/results/` にテキストで保存
...
```
https://github.com/harunamitrader/cli-prompt-cron のリポジトリをクローンして、skills/SKILL.md を読んでスキルを導入して。
```
...
- **Windows**: デスクトップに `.bat` ファイル（またはスタートメニューのショートカット）
- **Mac**: デスクトップに `.command` ファイル（ダブルクリックで Terminal が開いて起動）
...
1. cron デーモンが起動し、`~/.cli-prompt-cron/jobs/` のジョブを読み込む
...
`~/.cli-prompt-cron/jobs/<名前>.json` に以下の形式で作成します。
...
```
~/.cli-prompt-cron/logs/YYYY-MM-DD.log
```

```bash
# 今日のログを表示
cat ~/.cli-prompt-cron/logs/$(date +%Y-%m-%d).log

# リアルタイム監視
tail -f ~/.cli-prompt-cron/logs/$(date +%Y-%m-%d).log
```

### 実行結果

```
~/.cli-prompt-cron/results/
```

各ジョブの実行結果がテキストファイルとして保存されます。

```bash
ls ~/.cli-prompt-cron/results/
```
